...
int main(int argc, char ** argv) {
    ...
    if (argc < 4 || argv[1][0] == '-') {
        printf("usage: %s MODEL_PATH [PROMPT OR PROMPT_FILE] [VINEYARD_SOCKET] [N_GPU_LAYERS] [N_PREDICT] [N_BATCH]\n" , argv[0]);
        return 1 ;
    }
    ...
    std::shared_ptr<vineyard::KVStateCacheManager> manager;
    vineyard::Client client;
    if (vineyard_socket.size() > 0){
        VINEYARD_CHECK_OK(client.Connect(vineyard_socket));
        VINEYARD_CHECK_OK(vineyard::KVStateCacheManager::Make(client, manager, tensor_bytes,
                                              capacity, n_layers, block_size, 3));
    }
    std::vector<std::map<int, std::pair<vineyard::LLMKV, vineyard::LLMKV>>> kv_state_list;
    ...
    for (llama_seq_id seq_id = 0; seq_id < tokens_lists_size; seq_id++) {
        ...
        if (vineyard_socket.size() > 0){
            ...
            for (llama_pos prefill_pos = 0; prefill_pos < n_prefill; ++prefill_pos){
                for (uint32_t il = 0; il < n_layers; il++) {
                    vineyard::LLMKV key_state = kv_state_list[prefill_pos][il].first;
                    vineyard::LLMKV value_state = kv_state_list[prefill_pos][il].second;
        ...
        if (vineyard_socket.size() > 0){
            ...
            for (llama_pos i = token_start_pos; i < token_stop_pos; ++i) {
                std::map<int, std::pair<vineyard::LLMKV, vineyard::LLMKV>> new_kv_state;
