  ...
  meta_service_ptr_->RequestToGetData(
      ...
      [self, callback, object_id](const Status& status, const json& meta) {
        if (status.ok()) {
          ...
          std::cout << "Migrate object " << object_id << " to " << remote_endpoint
                << std::endl;
          // check if the object is already migrated
          // also ensure the migration is atomic, guaranteeing that each object_id will
          // only perform the migration process once.
