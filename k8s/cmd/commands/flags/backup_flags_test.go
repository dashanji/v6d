...
func TestApplyBackupCommonOpts(t *testing.T) {
	...
	t.Run("DefaultValues", func(t *testing.T) {
		...
		// 检查限制值是否正确设置为默认值
		if BackupOpts.Limit != 1000 {
			t.Errorf("Default limit value is incorrect, got: %d, want: 1000", BackupOpts.Limit)
		}
	...
	t.Run("CustomValues", func(t *testing.T) {
		// 设置自定义参数值
		cmd.SetArgs([]string{"--limit", "500", "--path", "/backup", "--pv-pvc-spec", "pv-pvc-spec-value"})

		// 执行命令
		err := cmd.Execute()

		// 检查限制值是否正确解析和设置
		if BackupOpts.Limit != 500 {
			t.Errorf("Custom limit value is incorrect, got: %d, want: 500", BackupOpts.Limit)
		}
